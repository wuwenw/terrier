...
class TPCHRunner : public benchmark::Fixture {
 public:
  ...
  void SetUp(const benchmark::State &state) final {
    ...
    db_oid_ = db_main_->GetCatalogLayer()->GetCatalog()->CreateDatabase(common::ManagedPointer<transaction::TransactionContext>(txn_), tpch_database_name_, true);
    accessor_ =
        db_main_->GetCatalogLayer()->GetCatalog()->GetAccessor(common::ManagedPointer<transaction::TransactionContext>(txn_), db_oid_, DISABLED);
...
auto l_returnflag =
    expr_maker.CVE(l_schema.GetColumn("l_returnflag").Oid(), type::TypeId::VARCHAR);
auto l_linestatus =
    expr_maker.CVE(l_schema.GetColumn("l_linestatus").Oid(), type::TypeId::VARCHAR);
